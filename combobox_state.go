...
func comboBoxProgress(gtx layout.Context, target float32, value, from, to *float32, at *time.Time, ready *bool) float32 {
	...
	progress := animationEase(animationProgress(gtx.Now.Sub(*at), comboBoxAnimationDuration))
	if progress < 1 {
...
func comboBoxItemScale(gtx layout.Context, history []widget.Press, disabled bool) float32 {
	...
	target := float32(0.98)
	if press.End.IsZero() {
		progress := animationEase(animationProgress(gtx.Now.Sub(press.Start), comboBoxItemPressInDuration))
		if progress < 1 {
			gtx.Execute(op.InvalidateCmd{})
		}
		return lerp(1, target, progress)
	}
	progress := animationEase(animationProgress(gtx.Now.Sub(press.End), comboBoxItemPressDuration))
	if progress < 1 {
		gtx.Execute(op.InvalidateCmd{})
	}
...
func comboBoxItemColor(gtx layout.Context, target color.NRGBA, value, from, to *color.NRGBA, at *time.Time, ready *bool) color.NRGBA {
	...
	progress := animationEase(animationProgress(gtx.Now.Sub(*at), comboBoxItemColorDuration))
	if progress < 1 {
		gtx.Execute(op.InvalidateCmd{})
	}

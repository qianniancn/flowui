...
func drawPopoverArrowShape(gtx layout.Context, placement PopoverPlacement, panel image.Point, arrow image.Point, style popoverStyle) {
	if arrow.X <= 0 || arrow.Y <= 0 {
		return
	}
	var path clip.Path
	path.Begin(gtx.Ops)
	switch popoverSide(placement) {
	case overlaySideTop:
		x := float32(panel.X) / 2
		y := float32(panel.Y)

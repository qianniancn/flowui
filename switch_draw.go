...
func drawSwitch(gtx layout.Context, theme *Theme, style switchStyle, size switchSizeStyle) switchDrawResult {
...
func switchRects(gtx layout.Context, theme *Theme, style switchStyle, size switchSizeStyle) switchDrawResult {
...
func drawSwitchFocus(gtx layout.Context, theme *Theme, rect image.Rectangle, style switchStyle) {
	if style.focus == 0 {
		return
	}
...
func drawSwitchTrack(gtx layout.Context, rect image.Rectangle, style switchStyle) {
	bg := lerpColor(style.trackOff, style.trackOn, style.selected)
	paint.FillShape(gtx.Ops, bg, clip.UniformRRect(rect, rect.Dy()/2).Op(gtx.Ops))
}

func drawSwitchThumb(gtx layout.Context, theme *Theme, rect image.Rectangle, style switchStyle, size switchSizeStyle) {
	if rect.Empty() {
		return
	}
	drawSwitchThumbShadow(gtx, theme, rect, size)
	bg := lerpColor(style.thumb, style.thumbOn, style.selected)
	paint.FillShape(gtx.Ops, bg, clip.UniformRRect(rect, rect.Dy()/2).Op(gtx.Ops))
}

func drawSwitchThumbShadow(gtx layout.Context, theme *Theme, rect image.Rectangle, size switchSizeStyle) {
	...
	DrawShadow(gtx, rect, switchThumbShadowShape(size), BoxShadow{
		Layers: []ShadowLayer{
...
func switchThumbShadowShape(size switchSizeStyle) ShadowShape {
	radius := size.thumbHeight / 2
	return RoundedShadowCorners(radius, radius, radius, radius)
}
...
func switchThumbContentColor(style switchStyle) color.NRGBA {
	return lerpColor(style.thumbFgOff, style.thumbFg, style.selected)
}

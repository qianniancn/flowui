...
func (p ProgressBarWidget) Layout(ctx *Context, gtx layout.Context) layout.Dimensions {
	state := ctx.progressBarState(p.key)
	style := progressBarStyleFor(ctx.Theme, p.color, p.disabled)
	sizeStyle := progressBarSizeStyleFor(ctx.Theme, p.size)

...
func drawRadio(gtx layout.Context, theme *Theme, style radioStyle, scale float32) layout.Dimensions {
...
func drawRadioControl(gtx layout.Context, theme *Theme, rect image.Rectangle, style radioStyle) {
	bg := lerpColor(style.bg, style.selectedBg, style.selected)
	border := style.border
...
func drawRadioDot(gtx layout.Context, theme *Theme, rect image.Rectangle, style radioStyle) {
	if style.selected == 0 {
		return
	}

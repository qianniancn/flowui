...
func (i InputWidget) layoutFrame(ctx *Context, gtx layout.Context, state *inputState, style inputStyle, child layout.Widget) layout.Dimensions {
	frameConstraints := gtx.Constraints
	if i.fullWidth {
		frameConstraints.Min.X = frameConstraints.Max.X
	}
	height := min(gtx.Dp(ctx.Theme.Spacing.ControlHeight), frameConstraints.Max.Y)
	...
	right := gtx.Dp(ctx.Theme.Components.Input.PaddingX)
	...
	radius := min(max(gtx.Dp(ctx.Theme.Shape.ControlRadius), 1), min(size.X, size.Y)/2)

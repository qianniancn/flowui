...
func (s *checkboxState) selection(gtx layout.Context, checked bool) float32 {
	...
	progress := render.Ease(render.Progress(gtx.Now.Sub(s.selectedAt), checkboxSelectDuration))
	if progress < 1 {
...
func (s *checkboxState) focusOpacity(gtx layout.Context, focused bool) float32 {
	...
	progress := render.Ease(render.Progress(gtx.Now.Sub(s.focusAt), checkboxFocusDuration))
	if progress < 1 {
		gtx.Execute(op.InvalidateCmd{})
	}

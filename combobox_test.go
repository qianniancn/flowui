...
func TestComboBoxInputValueOverridesSelectedLabel(t *testing.T) {
	...
	if got := ctx.combos["animal"].editor.Text(); got != "do" {
...
func TestComboBoxUpdatesChangedSelectedLabel(t *testing.T) {
	...
	if got := ctx.combos["animal"].editor.Text(); got != "Kitten" {
...
func TestComboBoxClearsRemovedSelectedItem(t *testing.T) {
	...
	if got := ctx.combos["animal"].editor.Text(); got != "" {
...
func layoutComboBoxFrame(ctx *Context, router *input.Router, combo ComboBoxWidget) {
	var ops op.Ops
	gtx := layout.Context{

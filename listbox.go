...
type ListBoxItem struct {
	...
	Leading     Widget
	Trailing    Widget
	Indicator   func(selected bool) Widget
	Disabled    bool
...
func (l ListBoxWidget) Layout(ctx *Context, gtx layout.Context) layout.Dimensions {
	state := ctx.listBoxState(l.key)
	state.beginFrame()
	...
	if !l.disabled {
		...
		if result.focusKey != "" {
			ctx.requestFocus(&state.item(result.focusKey).clickable)
		}
		if result.actionKey != "" {

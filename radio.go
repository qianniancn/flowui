...
func (r RadioGroupWidget) Layout(ctx *Context, gtx layout.Context) layout.Dimensions {
	state := ctx.radioGroupState(r.key)
	state.beginFrame()
	...
	if r.disabled {
		...
		ctx.requestFocus(&state.item(key).clickable)
	}
	...
	return layoutRadioItems(gtx, r.horizontal, gtx.Dp(theme.HorizontalGap), gtx.Dp(theme.VerticalGap), children)
}

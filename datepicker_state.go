...
func (s *datePickerState) popoverProgress(gtx layout.Context, open bool) float32 {
	...
	progress := animationEase(animationProgress(gtx.Now.Sub(s.popoverAt), s.popoverDuration))
	if progress < 1 {
		gtx.Execute(op.InvalidateCmd{})
	}
	s.popover = lerp(s.popoverFrom, s.popoverTo, progress)
	return s.popover
}
...
func datePickerColor(gtx layout.Context, target color.NRGBA, value, from, to *color.NRGBA, at *time.Time, ready *bool) color.NRGBA {
	...
	progress := animationEase(animationProgress(gtx.Now.Sub(*at), datePickerCellColorDuration))
	if progress < 1 {
		gtx.Execute(op.InvalidateCmd{})
	}
...
func datePickerPressScale(gtx layout.Context, history []widget.Press, disabled bool) float32 {
	...
	target := float32(0.95)
	if press.End.IsZero() {
		progress := animationEase(animationProgress(gtx.Now.Sub(press.Start), datePickerPressInDuration))
		if progress < 1 {
			gtx.Execute(op.InvalidateCmd{})
		}
		return lerp(1, target, progress)
	}
	progress := animationEase(animationProgress(gtx.Now.Sub(press.End), datePickerPressOutDuration))
	if progress < 1 {
		gtx.Execute(op.InvalidateCmd{})
	}

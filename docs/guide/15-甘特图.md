...
tasks := []ui.GanttChartTask{
	ui.GanttTask("design", "设计", day(0), day(5)).
		Group("产品").Progress(.8),
	ui.GanttTask("build", "开发", day(4), day(12)).
		Group("工程").DependsOn("design"),
	ui.GanttMilestone("launch", "发布", day(12)).
...
`GanttTask` 的结束时间必须晚于开始时间；零时长节点使用 `GanttMilestone`。`DependsOn` 使用任务 key，组件会拒绝空 key、未知任务、自依赖和依赖环。
...
```go
ui.GanttTask("platform", "平台", day(0), day(20)),
ui.GanttTask("api", "公共 API", day(2), day(10)).Parent("platform"),
ui.GanttTask("ui", "工作台", day(6), day(16)).Parent("platform"),
```
...
初始状态可以由 `GanttTask(...).Collapsed(true)` 设置。父任务引用必须存在，且不能形成父子循环。

## 时间窗口

```go
chart.TimeWindow(day(2), day(10)).OnTimeWindowChange(func(window ui.GanttTimeWindow) {
	// 保存或同步当前窗口
})
```

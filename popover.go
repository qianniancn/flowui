...
func (p PopoverWidget) Placement(placement PopoverPlacement) PopoverWidget {
...
func (p PopoverWidget) Layout(ctx *Context, gtx layout.Context) layout.Dimensions {
	triggerDims := p.layoutTrigger(ctx, gtx)
	fullKey := ctx.fullKey(p.key)
	if !p.open && !ctx.hasVisiblePopover(fullKey) {
		return triggerDims
	}

	state := ctx.popoverState(p.key)
	progress := state.progress(gtx, p.open)
	if p.open {
		p.handleCloseEvents(gtx, state)
	}
	if !p.open && progress <= 0 {
		delete(ctx.popovers, fullKey)
		return triggerDims
	}
...
func (p PopoverWidget) layoutTrigger(ctx *Context, gtx layout.Context) layout.Dimensions {
	if p.trigger == nil {

...
func TestDatePickerUsesContextLocale(t *testing.T) {
	ctx := newContextWithThemeAndLocale(nil, nil, DatePickerChinese())
...
func TestDatePickerInitialMonthClampsToRange(t *testing.T) {
	...
	if got := dateKey(ctx.datePickers["date"].viewMonth); got != "2026-07-01" {
	...
	if got := dateKey(ctx.datePickers["date"].viewMonth); got != "2026-08-01" {
...
func layoutDatePickerFrame(ctx *Context, router *input.Router, picker DatePickerWidget) layout.Dimensions {
	return layoutDatePickerFrameAt(ctx, router, picker, time.Time{})
}

func layoutDatePickerFrameAt(ctx *Context, router *input.Router, picker DatePickerWidget, now time.Time) layout.Dimensions {
	var ops op.Ops
	gtx := layout.Context{

...
func buttonAnimationScale(gtx layout.Context, history []widget.Press, theme *Theme, size ButtonSize, disabled bool) float32 {
	...
	if press.End.IsZero() {
		progress := animationEase(animationProgress(gtx.Now.Sub(press.Start), buttonPressInDuration))
		if progress < 1 {
			gtx.Execute(op.InvalidateCmd{})
		}
		return lerp(1, target, progress)
	}
	progress := animationEase(animationProgress(gtx.Now.Sub(press.End), buttonPressOutDuration))
	if progress < 1 {
		gtx.Execute(op.InvalidateCmd{})
	}
...
func buttonPressedScale(theme *Theme, size ButtonSize) float32 {
	switch size {
	case ButtonSmall:
...
func (s *buttonState) background(gtx layout.Context, target color.NRGBA) color.NRGBA {
	...
	progress := animationEase(animationProgress(gtx.Now.Sub(s.bgStart), buttonColorDuration))
	if progress < 1 {
		gtx.Execute(op.InvalidateCmd{})
	}
	s.bg = lerpColor(s.bgFrom, s.bgTo, progress)
	return s.bg
}
...
func (s *buttonState) focusOpacity(gtx layout.Context, focused bool) float32 {
	...
	progress := animationEase(animationProgress(gtx.Now.Sub(s.focusAt), buttonFocusDuration))
	if progress < 1 {
		gtx.Execute(op.InvalidateCmd{})
	}

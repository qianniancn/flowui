...
func drawListBoxItem(gtx layout.Context, theme *Theme, size image.Point, style listBoxItemStyle) {
...
func drawListBoxFocus(gtx layout.Context, theme *Theme, rect image.Rectangle, radius int, style listBoxItemStyle) {
	if style.focus == 0 {
		return
	}
...
func drawListBoxIndicator(gtx layout.Context, theme *Theme, size image.Point, style listBoxItemStyle) {
	if style.selected == 0 {
		return
	}
	...
	stroke := clip.Stroke{
		Path:  path,
		Width: max(dpFloat(gtx, theme.Components.ListBox.ItemIndicatorStroke), 1),
	}.Op().Push(gtx.Ops)
	paint.Fill(gtx.Ops, col)
	stroke.Pop()
}

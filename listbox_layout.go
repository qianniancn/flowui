...
func (l ListBoxWidget) layout(ctx *Context, gtx layout.Context, state *listBoxState, entries []listBoxEntry, hasItems bool) layout.Dimensions {
...
func (l ListBoxWidget) applyConstraints(ctx *Context, gtx *layout.Context) {
	if l.fullWidth {
		gtx.Constraints.Min.X = gtx.Constraints.Max.X
	}
	maxHeight := ctx.Theme.Components.ListBox.MaxHeight
	if l.maxHeight > 0 {
...
func (l ListBoxWidget) layoutSectionHeader(ctx *Context, gtx layout.Context, title string) layout.Dimensions {
	theme := ctx.Theme.Components.ListBox
	return layout.Inset{
...
func (l ListBoxWidget) layoutEmpty(ctx *Context, gtx layout.Context) layout.Dimensions {
	theme := ctx.Theme.Components.ListBox
...
func (l ListBoxWidget) layoutItem(ctx *Context, gtx layout.Context, state *listBoxState, item ListBoxItem) layout.Dimensions {
...
func (l ListBoxWidget) layoutIndicator(ctx *Context, gtx layout.Context, item ListBoxItem, style listBoxItemStyle, selected bool) layout.Dimensions {
	theme := ctx.Theme.Components.ListBox
	size := image.Pt(gtx.Dp(theme.ItemIndicatorSize), gtx.Dp(theme.ItemIndicatorSize))
	if item.Indicator == nil {
		drawListBoxIndicator(gtx, ctx.Theme, size, style)
...
func (l ListBoxWidget) layoutItemText(ctx *Context, gtx layout.Context, item ListBoxItem, style listBoxItemStyle) layout.Dimensions {
	theme := ctx.Theme.Components.ListBox
	if item.Description == "" {

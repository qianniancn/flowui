...
func (b BoxWidget) Layout(ctx *frame.Context, gtx layout.Context) layout.Dimensions {
	if b.interactive {
		click := interaction.BeginClick(ctx, gtx, b.key, nil, !b.disabled, true, b.onClick)
...
func (b BoxWidget) propertyStyle() flowstyle.Style {
	...
	if b.hasWidth {
		builder = builder.Width(b.width)
	}
	if b.hasHeight {
		builder = builder.Height(b.height)
	}
	if b.hasMinWidth {
		builder = builder.MinWidth(b.minWidth)
	}
	if b.hasMaxWidth {
		builder = builder.MaxWidth(b.maxWidth)
	}
	if b.hasMinHeight {
		builder = builder.MinHeight(b.minHeight)
	}
	if b.hasMaxHeight {
		builder = builder.MaxHeight(b.maxHeight)
	}
	if b.fillWidth {
		builder = builder.FillWidth()
	}
	if b.fillHeight {
		builder = builder.FillHeight()
	}
	if b.overflow == OverflowHidden {
		builder = builder.Overflow(flowstyle.OverflowHidden)
	}
	return builder
}
...
func (b BoxWidget) layoutVisual(ctx *frame.Context, gtx layout.Context, resolved flowstyle.ResolvedStyle) layout.Dimensions {
	...
	opacity := paint.PushOpacity(gtx.Ops, boxOpacity(resolved.Paint))
	defer opacity.Pop()

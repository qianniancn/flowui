...
func (p ProgressBarWidget) layout(ctx *Context, gtx layout.Context, style progressBarStyle, sizeStyle progressBarSizeStyle, progress float32) layout.Dimensions {
	output := p.outputText()
	...
	return layout.Flex{
		...
		Gap:  gtx.Dp(ctx.Theme.Components.ProgressBar.HeaderGap),
	}.Layout(gtx,
...
func (p ProgressBarWidget) layoutHeader(ctx *Context, gtx layout.Context, style progressBarStyle, output string) layout.Dimensions {
	children := make([]layout.FlexChild, 0, 3)
	if p.label != "" {
		children = append(children, layout.Rigid(func(gtx layout.Context) layout.Dimensions {
			return Text(p.label).
				Size(float32(ctx.Theme.Components.ProgressBar.TextSize)).
				Weight(font.Medium).
				Color(style.label).
				Layout(ctx, gtx)
		}))

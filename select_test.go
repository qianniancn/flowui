...
func TestSelectOptions(t *testing.T) {
	...
	if selectWidget.placement != PopoverTopStart || selectWidget.flipEnabled() || selectWidget.overflowAvoidanceEnabled() {
...
func TestSelectOpeningAnotherClosesActiveSelect(t *testing.T) {
	...
	firstState := ctx.selects["first"]
	secondState := ctx.selects["second"]
	if firstState == nil || secondState == nil || ctx.activeSelectKey != "first" {
	...
	if ctx.activeSelectKey != "second" {
		t.Fatalf("active select = %q, want second", ctx.activeSelectKey)
	}
}
...
func TestSelectOpeningAnotherRequestsControlledSelectClose(t *testing.T) {
	...
	ctx.selects["second"].trigger.Click()
...
func TestSelectPeerAndDismissRequestControlledCloseOnce(t *testing.T) {
	...
	ctx.selects = map[string]*selectState{
		"first":  first,
		"second": second,
	}
	ctx.activeSelectKey = "first"

	ctx.activateSelect(second)
...
func TestSelectActiveOwnershipClearsWhenStateIsSwept(t *testing.T) {
	...
	if ctx.activeSelectKey != "language" {
		t.Fatalf("active select = %q, want language", ctx.activeSelectKey)
	}

	ctx.beginFrame()
	ctx.endFrame()

	if ctx.activeSelectKey != "" {
		t.Fatalf("active select after sweep = %q, want empty", ctx.activeSelectKey)
	}
}
...
func TestSelectActiveOwnershipClearsWhenSelectCloses(t *testing.T) {
	...
	if ctx.activeSelectKey != "" {
		t.Fatalf("active select after close = %q, want empty", ctx.activeSelectKey)
	}
}
...
func TestSelectArrowDownOpensAndFocusesFirstOption(t *testing.T) {
	...
	listState := ctx.listBoxes["language:options"]
	if listState == nil || listState.items["rust"] == nil {
		t.Fatal("select options were not laid out")
	}
	if !router.Source().Focused(&listState.items["rust"].clickable) {
		t.Fatal("ArrowDown did not focus first enabled option")
	}
	if listState.items["rust"].pointerFocus {
	...
	if listState.items["rust"].focusTo != 1 {
...
func TestSelectPointerOpenFocusesSelectedWithoutFocusRing(t *testing.T) {
	...
	listState := ctx.listBoxes["language:options"]
	if listState == nil || listState.items["go"] == nil {
		t.Fatal("pointer-opened select did not lay out its selected option")
	}
	itemState := listState.items["go"]
	if !router.Source().Focused(&itemState.clickable) {
		t.Fatal("pointer-opened select did not focus its selected option")
	}
	if !itemState.pointerFocus {
...
func TestSelectEscapeClosesFromOptionAndRestoresTriggerFocus(t *testing.T) {
	...
	state := ctx.selects["language"]
	listState := ctx.listBoxes["language:options"]
	if state == nil || listState == nil || !router.Source().Focused(&listState.items["go"].clickable) {
...
func TestSelectSingleSelectionClosesAndMultipleStaysOpen(t *testing.T) {
	t.Run("single", func(t *testing.T) {
		...
		listState := &listBoxState{items: map[string]*listBoxItemState{"rust": itemState}}
		ctx := newContext(nil)
		ctx.listBoxes = map[string]*listBoxState{"language:options": listState}
		state := &selectState{open: true}
		changed := ""
	...
	t.Run("multiple", func(t *testing.T) {
		...
		listState := &listBoxState{items: map[string]*listBoxItemState{"rust": itemState}}
		ctx := newContext(nil)
		ctx.listBoxes = map[string]*listBoxState{"languages:options": listState}
		state := &selectState{open: true}
		var changed []string
...
func TestSelectFocusPendingOptionRequestsListItemFocus(t *testing.T) {
	goItem := new(listBoxItemState)
	rustItem := new(listBoxItemState)
	ctx := newContext(nil)
	ctx.listBoxes = map[string]*listBoxState{
		"language:options": {items: map[string]*listBoxItemState{"go": goItem, "rust": rustItem}},
	}
	state := &selectState{focusIntent: selectFocusSelected}
	widget := Select("language", "rust", selectTestItems())
	gtx, router := selectFocusTestContext()
	rustItem.clickable.Layout(gtx, func(gtx layout.Context) layout.Dimensions {
		return layout.Dimensions{Size: image.Pt(20, 20)}
	})
	...
	if !router.Source().Focused(&rustItem.clickable) {
...
func TestSelectMultipleValueWrapsAndGrowsTrigger(t *testing.T) {
	...
	if dims.Size.Y <= gtx.Dp(ctx.Theme.Components.Select.Height) {
...
func TestSelectStylesMatchPrimaryAndSecondaryFields(t *testing.T) {
	...
	if secondary.field.shadowOpacity != 0 || secondary.field.bg != theme.Palette.SurfaceRaised {
...
func layoutSelectTestFrame(ctx *Context, router *input.Router, widget SelectWidget, now time.Time) {
	var ops op.Ops
	gtx := layout.Context{
...
func layoutSelectPairTestFrame(ctx *Context, router *input.Router, first, second SelectWidget, now time.Time, reverse bool) {
	var ops op.Ops
	gtx := layout.Context{
		Constraints: layout.Constraints{Max: image.Pt(300, 240)},
		Source:      router.Source(),
...
func layoutNestedSelectTestFrame(ctx *Context, router *input.Router, selectWidget SelectWidget, background *widget.Clickable, backgroundClicked *bool, now time.Time) {
	var ops op.Ops
	...
	ctx.beginFrameWithViewport(viewport)
	presses := activePresses(background.History())
	for background.Clicked(gtx) {
		*backgroundClicked = true
	}
	ctx.focusOnPress(background, background.History(), presses)

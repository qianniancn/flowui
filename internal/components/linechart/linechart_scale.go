...
func newLinearScale(minimum, maximum float64, targetTicks int, includeZero, fixed bool) linearScale {
	...
	return linearScale{minimum: scaleMinimum, maximum: scaleMaximum, interval: interval, ticks: ticks}
}

func niceNumber(value float64, rounded bool) float64 {
	// Use the 1/2/3/5/10 interval strategy from ECharts 6.1.0, based on
	// "Nice Numbers for Graph Labels" from Graphics Gems.

...
func TestSwitchSyncsValue(t *testing.T) {
	...
	state := ctx.switches["notifications"]
	if state == nil {
...
func TestSwitchThumbReceivesCheckedState(t *testing.T) {
	got := false
	Switch("power", true, "").
		Thumb(func(checked bool) Widget {
			got = checked
			return Text("1")
		}).
...
func (f fixedSwitchChild) Layout(_ *Context, gtx layout.Context) layout.Dimensions {
...
func layoutSwitchFrame(ctx *Context, router *input.Router) {
	layoutSwitchFrameWith(ctx, router, Switch("notifications", false, "Notifications"))
}

func layoutSwitchFrameWith(ctx *Context, router *input.Router, sw SwitchWidget) {
	var ops op.Ops
	gtx := layout.Context{

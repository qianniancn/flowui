...
type SelectItem = ListBoxItem
type SelectSection = ListBoxSection
type SelectVariant = InputVariant

const (
	SelectPrimary   SelectVariant = InputPrimary
	SelectSecondary SelectVariant = InputSecondary
)
...
func (s SelectWidget) Indicator(indicator Widget) SelectWidget {
...
func (s SelectWidget) Placement(placement PopoverPlacement) SelectWidget {
...
func (s SelectWidget) Layout(ctx *Context, gtx layout.Context) layout.Dimensions {
	...
	if open && !state.wasOpen {
		ctx.activateSelect(state)
	} else if !open {
		ctx.releaseSelect(state)
	}
